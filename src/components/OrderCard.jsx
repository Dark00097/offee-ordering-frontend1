...
function OrderCard({ order, onUpdateStatus, onApproveOrder, timeAgo, isExpanded: initialExpanded = false }) {
  ...
  const groupedItems = (() => {
    ...
    const itemIds = order.item_ids?.split(',').filter(id => id.trim()) || [];
    const itemNames = order.item_names?.split(',') || [];
    const unitPrices = order.unit_prices?.split(',').map(price => parseFloat(price) || 0) || [];
    const menuQuantities = order.menu_quantities?.split(',').filter(q => q !== 'NULL') || [];
    ...
    const breakfastIds = order.breakfast_ids?.split(',').filter(id => id.trim()) || [];
    const breakfastNames = order.breakfast_names?.split(',') || [];
    const breakfastQuantities = order.breakfast_quantities?.split(',').filter(q => q !== 'NULL') || [];
    ...
    const optionIds = order.breakfast_option_ids?.split(',').filter(id => id.trim()) || [];
    ...
    breakfastIds.forEach((id, idx) => {
      ...
      for (let i = startIdx; i < endIdx && i < optionIds.length; i++) {
        if (order.breakfast_option_ids[i]) {
        ...
        <div style={styles.expandedContent}>
          ...
          <div style={styles.totalSection}>
            <div style={styles.totalRow}>
              <span>Total</span>
              <span style={styles.totalValue}>${parseFloat(order.total_price).toFixed(2)}</span>
            </div>
          </div>
